  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        ...
        {task.prompt && (
          <section>
            <div className="text-caption-xs text-txt-tertiary uppercase tracking-wider mb-2">Prompt</div>
            <div className="section-container px-4 py-3">
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <span className="text-caption-xs text-txt-tertiary uppercase tracking-wider">Logs</span>
            <Button size="sm" onClick={loadLogs} disabled={logsLoading}>
              {logsLoading ? 'Loading...' : logs !== null ? 'Reload' : 'Load Logs'}
            </Button>
          </div>
          {logsError && (
            <div className="text-caption-xs text-semantic-red bg-semantic-red-soft border border-semantic-red/20 rounded-lg px-3 py-2 mb-2">{logsError}</div>
          )}
          {logs !== null && (
            <pre className="text-caption-xs text-txt-tertiary font-mono whitespace-pre-wrap bg-bg-surface border border-border-subtle rounded-lg p-3 overflow-x-auto max-h-[400px] overflow-y-auto">
              {logs || '(empty)'}
            </pre>
          )}
        </section>
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <form onSubmit={handleSubmit} className="max-w-[480px] space-y-3">
          {error && (
            <div className="text-caption-xs text-semantic-red bg-semantic-red-soft border border-semantic-red/20 rounded-lg px-3 py-2">{error}</div>
          )}
